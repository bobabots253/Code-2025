...
/* Important Notes for 2025 (On-Season Brainstorm):
...
Goals.
  Aspects with Significance Influence:
...
public class Robot extends TimedRobot {
  ...
  public void autonomousInit() {
    ...
    Pose2d RedEFAlgaePose2d = new Pose2d((new Translation2d(11.850906, 4.0200)),
                                              new Rotation2d(0 * (Math.PI/180)));//testing pose
    ...
    if (AutoModeManager.m_autonomousCommand == null){
      if(AutoModeManager.desiredMode == AutoModeManager.DesiredMode.L1_MIDDLE_START){
        ...
        if(DriverStation.getAlliance().get() == Alliance.Blue){
          m_robotContainer.ReturnL2StaggeredCommand(blueESideKickOffPose2d, blueICoralPose2d, 2.0, 4, 2).schedule();
        }else{
          m_robotContainer.ReturnL2StaggeredCommand(redESideKickOffPose2d, redICoralPose2d, 2.0, 4, 2).schedule();
        }
      }else if(AutoModeManager.desiredMode == AutoModeManager.DesiredMode.E_SIDE_RIGHT_L2){
        if(DriverStation.getAlliance().get() == Alliance.Blue){
          m_robotContainer.ReturnL2StaggeredCommand(blueESideKickOffPose2d, blueJCoralPose2d, 2.0, 4, 2).schedule();
        }else{
          m_robotContainer.ReturnL2StaggeredCommand(redESideKickOffPose2d, redJCoralPose2d, 2.0, 4, 2).schedule();
        }
