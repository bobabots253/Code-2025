...
public class AutoAlignTwo extends Command {
  private PIDController pidX = new PIDController(0.4, 0, 0);
  private PIDController pidY = new PIDController(0.4, 0, 0);
...
//           .withDriveRequestType(DriveRequestType.OpenLoopVoltage);
  ...
  public void initialize() {
    ...
    Pose2d robotPose = DriveSubsystem.getInstance().getRefinedPoseVision();
  ...
  @Override
  public void execute() {
    Pose2d currentPose = DriveSubsystem.getInstance().getRefinedPoseVision();
    ...
    ChassisSpeeds request = new ChassisSpeeds(powerX, powerY, powerRotate);
    SwerveModuleState[] requestedStates =
             DriveSubsystem.getInstance().m_kinematics.toSwerveModuleStates(request);
    DriveSubsystem.getInstance().setModuleStates(requestedStates);
