...
public class ElevatorSubsystem extends SubsystemBase{
    ...
    private ElevatorSubsystem() {
        ...
        m_profiledPIDController = new ProfiledPIDController(
            ElevatorConstants.profiledP, 
            ElevatorConstants.profiledI, 
            ElevatorConstants.profiledD, 
            new TrapezoidProfile.Constraints(ElevatorConstants.elevatorMaxVelocity, ElevatorConstants.elevaotrMaxAccerleration),
            0.02
        );
        m_profiledPIDController.setIZone(0);

        m_masterLiftingSparkMax.configure(ElevatorConfigs.masterLiftingConfig, ResetMode.kResetSafeParameters,
        PersistMode.kPersistParameters);
        m_slaveLiftingSparkMax.configure(ElevatorConfigs.slaveLiftingConfig, ResetMode.kResetSafeParameters,
        PersistMode.kPersistParameters);
}
...
// @Override
    ...
    public void setTargetExtension(double desiredPos){
        ...
        trapezoid = m_profiledPIDController.calculate(rotToMeters(m_LiftingEncoder.getPosition()), rotToMeters(desiredPos))+ .7; // doesn't match up with the other one???
    }

    private double rotToMeters(double rot){
        return (((rot/ElevatorConstants.gearRatio)/(Math.PI*2*ElevatorConstants.gearRadius))/2);
    }
